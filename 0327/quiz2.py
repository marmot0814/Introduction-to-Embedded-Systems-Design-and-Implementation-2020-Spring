...
class ADXL345(IMU):
    ...
    def getPitch(self) :
        ...
        aY = self.getYg()
        aZ = self.getZg()
        self.pitch = np.arctan(-aX / np.sqrt(aY * aY + aZ * aZ))
        return self.pitch 
    ...
    def getTilt(self) :
        ...
        aY = self.getYg()
        aZ = self.getZg()
        self.tilt = np.arccos(aZ / np.sqrt(aX * aX + aY * aY + aZ * aZ))
        return self.tilt
...
try:
    ...
    while True:
        ...
        adxl345.getY()
        adxl345.getZ()
        print ("roll: {}".format(adxl345.getRoll()))
        print ("pitch: {}".format(adxl345.getPitch()))
        print ("tilt: {}".format(adxl345.getTilt()))
        print ("")
        time.sleep(1)
