...
class ADXL345(IMU):
    ...
    def getPitch(self) :
        ...
        aY = self.getYg()
        aZ = self.getZg()
        self.pitch = np.arctan(-aX / np.sqrt(pow(aY, 2) + pow(aZ, 2)))
        return self.pitch 
    ...
    def getTilt(self) :
        ...
        aY = self.getYg()
        aZ = self.getZg()
        self.tilt = np.arccos(aZ / np.sqrt(pow(aX, 2) + pow(aY, 2) + pow(aZ, 2)))
        return self.tilt
...
try:
    ...
    while True:
        ...
        pitch = 0.98 * (pitch + gyro.getXangle() * time.clock()) + 0.02 * adxl345.getPitch()
        roll = 0.98 * (roll + gyro.getYangle() * time.clock()) + 0.02 * adxl345.getRoll()
        clock = time.clock()
